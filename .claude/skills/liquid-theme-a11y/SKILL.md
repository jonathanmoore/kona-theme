...
Use Tailwind's `sr-only` and `focus:not-sr-only` utilities for skip links:

```html
<a href="#main-content" class="sr-only focus:not-sr-only focus:fixed focus:inset-0 focus:z-50 focus:flex focus:items-center focus:justify-center focus:bg-contrast focus:text-primary">
  {{ 'accessibility.skip_to_content' | t }}
</a>
```
...
Use Tailwind's `focus-visible:` variant for consistent focus styling:
...
For global focus styles, add to `base.css`:
...
This theme provides `trapFocus` and `removeTrapFocus` in `@/lib/a11y`:
...
open(trigger) {
  ...
  trapFocus(this.querySelector('.dialog-container'))
}
...
See [focus and keyboard patterns](references/focus-and-keyboard.md) for full implementation details.
...
```html
<article class="group relative" aria-labelledby="ProductTitle-{{ product.id }}">
  <a href="{{ product.url }}" aria-labelledby="ProductTitle-{{ product.id }}">
    <img
  ...
  <div aria-label="{{ 'products.price_label' | t: price: product.price | money }}">
    {{ product.price | money }}
  </div>
  <button
    tabindex="-1"
...
- Uses island hydration: `<cart-drawer client:visible>`
...
<form action="{{ routes.cart_url }}" method="post">
  <div class="flex flex-col gap-1">
    <label for="Email-{{ section.id }}" class="text-fine">{{ 'forms.email' | t }}</label>
    <input
      type="email"
      id="Email-{{ section.id }}"
      name="email"
      ...
      class="rounded border border-primary/10 px-3 py-2 focus-visible:outline-2 focus-visible:outline-primary"
    >
    <p
      id="EmailError-{{ section.id }}"
      role="alert"
      hidden
      class="text-notice text-fine"
...
<form class="space-y-4">
  <div>
    ...
    <fieldset id="FilterColor-{{ section.id }}" hidden>
      <legend class="sr-only">{{ 'filters.filter_by_color' | t }}</legend>
      {% for color in colors %}
...
{% if product.compare_at_price > product.price %}
  <div aria-label="{{ 'products.sale_price_label' | t: sale_price: product.price | money, original_price: product.compare_at_price | money }}">
    <s aria-hidden="true" class="text-primary/50">{{ product.compare_at_price | money }}</s>
...
```html
<details>
  <summary class="cursor-pointer py-3 font-heading">{{ block.settings.heading }}</summary>
  <div class="pb-4">
...
- **Touch targets:** minimum 44x44px (`min-h-11 min-w-11`), 8px spacing between targets
...
Use Tailwind's `motion-reduce:` variant for reduced motion:

```html
<div class="transition-transform duration-200 motion-reduce:transition-none motion-reduce:transform-none">
  {{ content }}
</div>
```

For global reduced motion in CSS:
...
Use Tailwind's `sr-only` class instead of a custom `.visually-hidden`:

```html
<span class="sr-only">{{ 'accessibility.screen_reader_text' | t }}</span>
```
