...
This theme provides `trapFocus` and `removeTrapFocus` in `@/lib/a11y`. Use these in island components:
...
class MyDialog extends HTMLElement {
  ...
  open(trigger) {
    ...
    const container = this.querySelector('.dialog-container')
...
For a manual focus trap implementation:
